...
# from openai_llm_phi_vllm import vllm_get_response
from openai_llm import oai_get_response
import text_util

class FeedbackLoop:
    def __init__(self, model, pred_cui_pairs_set, sents_db, evaluator):
        self.model = model
    ...
    def feedback_loop_all_paths(self, top_20_df, max_iterations=3):
        ...
        for idx, row in top_20_df.iterrows():
            ...
            source_cui = 'amiodarone'  # 'C1420213'
            target_cui = 'mefloquine'  # 'C0085208'
            ...
            for iteration in range(max_iterations):
                ...
                try:
                    ...
                    # prompt, temp, top_p, model, model_url, max_tokens=2200, seed=1234):
                    llm_resp_path = oai_get_response(llm_fix_prompt_combo,temp, top_p, seed=1234)
                    end_time = time.time()
                    print(f"Response from VLLM at iteration {iteration + 1}:\n{llm_resp_path}")
                    print(f"Time taken for VLLM LOCAL PHI response: {end_time - start_time:.2f} seconds")
                ...
                start_time = time.time()
                eval_result_path = self.evaluator.evaluate_llm_resp(llm_resp_path)
                end_time = time.time()
                print(f"Time saeideh taken for evaluation llm response: {end_time - start_time:.2f} seconds")
